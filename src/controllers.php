...
$app->match('/todo/edit/{id}', function ($id) use ($app)
{
    ...
    // print_r($todo);exit;
    return $app['twig']->render('edit.html', ['todo' => $todo, ]);
...
$app->post('/todo/update', function (Request $request) use ($app)
{
    ...
    if (count($errors) == 0)
    {
        ...
        # Set flash message and return redirect
        $app['session']->getFlashBag()
            ->add('success_msg', 'The task has been updated successfully.');
        return $app->redirect('/todo');
...
$app->match('/todo/task_complete/{id}', function ($id) use ($app)
{
    ...
    # Set flash message and return redirect
    $app['session']->getFlashBag()
        ->add('success_msg', 'The task has been marked as completed successfully.');
    return $app->redirect('/todo');
